    ...
    try {
      const testResultData: any = {
        ...
        test_data: {
          ...
          fixationCapture: scanResult.fixationCapture ? {
            durationMs: scanResult.fixationCapture.durationMs,
            totalFrames: scanResult.fixationCapture.summary.totalFrames,
            goodFrames: scanResult.fixationCapture.summary.goodFrames,
            averageQuality: scanResult.fixationCapture.summary.averageQuality,
            usedSensorData: scanResult.fixationCapture.summary.usedSensorData,
            screeningNote: scanResult.fixationCapture.screeningNote,
          } : null,
          alignment: {
          ...
          pupilExamination: scanResult.pupilExamination ? {
            meanDiameterMM: scanResult.pupilExamination.meanDiameterMM,
            asymmetryDetected: scanResult.pupilExamination.asymmetryDetected,
            reactivityDetected: scanResult.pupilExamination.reactivityDetected,
            screeningNote: scanResult.pupilExamination.screeningNote,
            dataQuality: scanResult.pupilExamination.dataQuality,
            usedSensorData: scanResult.pupilExamination.usedSensorData,
          } : null,
          qualityAssessment: {
...
• Convergence: ${result.convergence.nearPoint ? result.convergence.nearPoint.toFixed(0) + 'mm' : 'Inconclusive'}${result.pupilExamination ? `
• Pupils: ${result.pupilExamination.meanDiameterMM.left.toFixed(1)}mm (L) / ${result.pupilExamination.meanDiameterMM.right.toFixed(1)}mm (R) - ${result.pupilExamination.asymmetryDetected ? 'Asymmetric' : 'Symmetric'}` : ''}
      ...
      <View style={styles.content}>
        ...
        <View style={styles.modulesCard}>
          ...
          {result.fixationCapture && (
            <View style={styles.moduleRow}>
              <View style={styles.moduleHeader}>
                <Text style={styles.moduleName}>Fixation Capture</Text>
                <Text style={[styles.moduleStatus, { color: result.fixationCapture.summary.averageQuality >= 0.6 ? '#10B981' : '#F59E0B' }]}>
          ...
          {result.pupilExamination && (
            <View style={styles.moduleRow}>
              <View style={styles.moduleHeader}>
                <Text style={styles.moduleName}>Pupil Examination</Text>
                <Text style={[styles.moduleStatus, { 
                  color: result.pupilExamination.asymmetryDetected || !result.pupilExamination.reactivityDetected ? '#F59E0B' : '#10B981' 
                }]}>
                  {result.pupilExamination.asymmetryDetected ? 'Asymmetric' : 'Symmetric'}
                </Text>
              </View>
              <Text style={styles.moduleNote}>
                {result.pupilExamination.meanDiameterMM.left.toFixed(1)}mm (L) / {result.pupilExamination.meanDiameterMM.right.toFixed(1)}mm (R)
                {'\n'}{result.pupilExamination.screeningNote}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={[
          ...
          { 
            backgroundColor: result.deviceTier === 'precision' || result.deviceTier === 'depth' ? '#D1FAE5' : '#FEF3C7',
            borderColor: result.deviceTier === 'precision' || result.deviceTier === 'depth' ? '#10B981' : '#F59E0B'
          }
        ]}>
          <View style={styles.capabilityHeader}>
            <Text style={styles.capabilityIcon}>
              {result.deviceTier === 'precision' ? '⚡' : result.deviceTier === 'depth' ? '✓' : '⚠️'}
            </Text>
            <View style={styles.capabilityTextContainer}>
              <Text style={[
                styles.capabilityTitle,
                { color: result.deviceTier === 'precision' || result.deviceTier === 'depth' ? '#065F46' : '#92400E' }
              ]}>
                {result.deviceTierInfo.name} Tier
              </Text>
              <Text style={[
                styles.capabilityText,
                { color: result.deviceTier === 'precision' || result.deviceTier === 'depth' ? '#065F46' : '#92400E' }
              ]}>
                {result.deviceTierInfo.description}
              </Text>
              <Text style={[
                styles.capabilityText,
                { color: result.deviceTier === 'precision' || result.deviceTier === 'depth' ? '#065F46' : '#92400E', marginTop: 8 }
              ]}>
                Accuracy: Alignment {result.deviceTierInfo.measurementAccuracy.alignment}, 
                Motility {result.deviceTierInfo.measurementAccuracy.motility}, 
                Convergence {result.deviceTierInfo.measurementAccuracy.convergence}
              </Text>
            </View>
          </View>
        </View>
